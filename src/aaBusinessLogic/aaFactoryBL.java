...
public class aaFactoryBL<T>  {
    private final IDAO<T> oDAO;

    public aaFactoryBL(Class<? extends IDAO<T>> classDAO) {
        try {
            this.oDAO = classDAO.getDeclaredConstructor().newInstance();
        } catch (Exception e) {
            AppException er = new AppException("Error al instanciar classDAO<T>", e, getClass(), "aaFactoryBL(...)");
